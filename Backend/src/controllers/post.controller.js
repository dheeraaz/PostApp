...
const createPost = asyncHandler(async (req, res) => {
  ...
  res
    .status(200)
    .json(new apiResponse(200, newPost, "Successfully created the post"));
});
...
const getAllPosts = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(
      new apiResponse(200, allPosts, "Successfully fetched all posts data")
    );
});
...
const getUserPosts = asyncHandler(async (req, res) => {
  ...
  res
    .status(200)
    .json(new apiResponse(200, userPosts, "Successfully fetched user's post"));
});
...
const deletePost = asyncHandler(async (req, res) => {
  ...
  if (postToBeDeleted?.postimgs.length > 0) {
    const imagesToBeDeleted = postToBeDeleted.postimgs.map((img) => {
  ...
  if (!deletePost)
    return res
      .status(500)
      .json(new apiError(500, "Cannot delete post at this moment"));

  return res
    .status(200)
    .json(new apiResponse(204, {}, "Successfully deleted the post"));
});
...
const getSinglePost = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new apiResponse(200, post, "Successfully fetched post"));
});
...
const updatePost = asyncHandler(async (req, res) => {
  ...
  if (previousimgs.length > 0 && prevPost?.postimgs?.length > 0) {
    ...
    imagesToBeDeleted = prevPost.postimgs.map((img) => {
      return img.public_id;
    });
  }
  ...
  res
    .status(200)
    .json(new apiResponse(200, updatedPost, "Successfully updated the post"));
});
...
const toggleLike = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new apiResponse(200, populatedPost, "Successfully liked the post"));
});
